...
var moduleManager = module.exports = function(options) {
  ...
  app.configure(function(){
    ...
    app.use(require('./format_resolver'));
    app.use(app.router);
...
moduleManager.install = function(mods) {
  if (mods && mods.length > 0) mods.forEach(function(name){
    findModule(name).install();
  });
  else moduleManager._rootModule.installAll();
};
moduleManager.uninstall = function(mods) {
  if (mods && mods.length > 0) mods.forEach(function(name){
    findModule(name).uninstall();
  });
  else moduleManager._rootModule.uninstallAll();
};
moduleManager.enable = function(mods) {
  if (mods && mods.length > 0) mods.forEach(function(name){
    findModule(name).enable();
  });
  else moduleManager._rootModule.enableAll();
};
moduleManager.disable = function(mods) {
  if (mods && mods.length > 0) mods.forEach(function(name){
    findModule(name).disable();
  });
  else moduleManager._rootModule.disableAll();
};
