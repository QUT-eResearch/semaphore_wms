...
renderers.default = function(res) {
...
for (var key in fieldFormatter) {
  helper.setField({view:'show', type:key}, fieldFormatter[key]);
  helper.setField({view:'list', type:key}, fieldFormatter[key]);
}

helper.setForm('', function() {return '';});
helper.setForm('show', getCompiledView('partials/fields_show'));
...
for (var key in fieldForm) {
  helper.setField({view:'new', type:key}, fieldForm[key]);
  helper.setField({view:'edit', type:key}, fieldForm[key]);
}

function createFormOptions(type, name, value) {
  return {
    name: name,
    type: type,
    value: value,
...
function inputText(opt) {
  ...
  var options = createFormOptions('text', opt.field.name, opt.value);
  return fieldFormInput(options);
}

function inputCheckbox(opt) {
  var options = createFormOptions('checkbox', opt.field.name, 'true');
...
function formatArray(opt) {
  ...
  opt.value.forEach(function(item, index) {
    if (index > 0) str += ', ';
    var field = {name:opt.name, type:opt.of};
    str += fieldFormatter[opt.of]({field:field, value:item});
  });
  str += ']';
  return str;
}
