...
/** Resolve keys in order of :
module.view.fieldName
module.fieldName
view.fieldName
fieldName
module.view.fieldType
module.fieldType
view.fieldType
fieldType
module.view
...
helper.field = helper.prototype.field = function(field, value, context) {
  var prefix = '_field';
  context = context || {};
  var modns = this.entity.owner ? this.entity.owner.namespace : '';
  var mod = context.module || modns || '';
  var view = context.view || this.view || '';
  var keys = [];
  [ '!name:' + field.name , '!type:' + field.type ].forEach(function(suffix) {
    if (mod) {
      if (view) keys.push(prefix + '!mod:'+mod + '!view:'+view + suffix);
      keys.push(prefix + '!mod:'+mod + suffix);
    }
    if (view) keys.push(prefix + '!view:'+view + suffix);
    keys.push(prefix + suffix);
  });
  if (mod) {
    if (view) keys.push(prefix + '!mod:'+mod + '!view:'+view);
    keys.push(prefix + '!mod:'+mod);
  }
  if (view) keys.push(prefix + '!view:'+view);
  keys.push(prefix);
  var renderField;
  keys.some(function(key){
    renderField = helper.partials[key];
    return renderField;
  });
  return renderField({field: field, value: value, context: {module:mod, view:view}});
};

helper.setField = function(context, value) {
  var key = '_field';
  if (context.module) key += ('!mod:' + context.module);
  if (context.view) key += '!view:' + context.view;
  if (context.name) key += '!name:' + context.name;
  else if (context.type) key += '!type:' + context.type;
  helper.partials[key] = value;
};

helper.setField({}, function() {return '';});
