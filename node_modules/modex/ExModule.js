...
ExModule.prototype.install = function(options) {
  if (this.isGroup() || this.isInstalled || !this.parent.isInstalled) return;
  try {
    if (this.main && this.main.install) this.main.install(options);
    this._state |= FLAG_INSTALLED;
    stateModified = true;
  } catch (err) {
    if (logger) logger.error("Failed executing install method in module %s", this.name);
  }
};

ExModule.prototype.installAll = function(options) {
  this.install(options);
  for (var key in this.modules) {
    this.modules[key].installAll(options);
  }
};

ExModule.prototype.uninstall = function(options) {
  if (this.isGroup() || !this.isInstalled || this.hasInstalledSubmodule()) return;
  this.disable(true);
  try {
    if (this.main && this.main.uninstall) this.main.uninstall(options);
    this._state &= ~FLAG_INSTALLED;
    stateModified = true;
  } catch (err) {
    if (logger) logger.error("Failed executing uninstall method in module %s", this.name);
  }
};

ExModule.prototype.uninstallAll = function(options) {
  for (var key in this.modules) {
    this.modules[key].uninstallAll(options);
  }
  this.uninstall(options);
};

ExModule.prototype.enable = function() {
  if (this.isGroup() || !this.isInstalled || !this.parent.isEnabled || this.isEnabled) return;
  this._state |= FLAG_ENABLED;
  this._init();
  stateModified = true;
};

ExModule.prototype.enableAll = function() {
  this.enable();
  for (var key in this.modules) {
    this.modules[key].enableAll();
  }
};

ExModule.prototype.disable = function() {
  if (this.isGroup() || !this.isEnabled || this.hasEnabledSubmodule()) return;
  this._state &= ~FLAG_ENABLED;
  this._deinit();
  stateModified = true;
};

ExModule.prototype.disableAll = function() {
  for (var key in this.modules) {
    this.modules[key].disableAll();
  }
  this.disable();
};
...
ExModule.prototype._load = function(state) {
  ...
  if (this.isEnabled) {
    this.addModels('./models', true);
    this.addViews('./views', true);
    this.addControllers('./controllers', true);
...
ExModule.prototype.mount = function mount(method, route, params, callbacks) {
  ...
  try {
    ...
    console.log(e.stack);
    logger.error('Initialize module [%s]: mount() : %s', mod.namespace, e);
  }
};
...
ExModule.prototype.mountController = function mountController(controller, params) {
  ...
  base.module.registerResource(mod, params.label, routeBase);
...
ExModule.prototype.addController = function addController(controller, params) {
  ...
  if (typeof controller._init === 'function') controller._init(this, moduleManager);
  logger.debug('{%s} load and init controller `%s`', this.namespace, name);
};

/** Load all js files from a directory. Non recursive */
ExModule.prototype.addControllers = function addControllers(pathToDir, params, ignoreError) {
  ...
  try {
    ...
    if (!ignoreError) {
      logger.error('{%s} Error loading controllers: %s', this.namespace, pathToDir);
      logger.error(err);
    }
  }
...
ExModule.prototype.addModels = function addModels(pathToDir, ignoreError) {
  var basePath = path.resolve(this.path, pathToDir);
  var mod = this;
  try {
...
ExModule.prototype.addViews = function addViews(pathToDir, ignoreError) {
...
ExModule.prototype.addStatic = function(pathToDir, name) {
  var staticPath = path.resolve(this.path, pathToDir);
  if (!name) name = path.basename(pathToDir);
  var route = this.routePathTo('/'+name);
